...
def command_executor(cmd, verbose=False, msg=None, stdout=sys.stdout, stderr=sys.stderr):
    '''
    ...
    try:
        process = subprocess.Popen(shlex.split(cmd), stdout=stdout, stderr=stderr)
        so, se = process.communicate()
        retcode = process.returncode
        assert retcode == 0, "Error in running command: " + cmd
    except OSError as e:
        print("!!! Execution failed !!!", e, " -->  Error running command:", cmd, file=sys.stderr)
        raise
