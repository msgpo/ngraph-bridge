...
Status EncapsulateClusters(
  ...
  if (!analysis_pass) {
    // In analysis pass our target is to just get the TF subgraphs, so we do not
    // want to rewrite the original graph in any way
    for (auto& kv : device_name_map) {
      ...
      for (auto& tup : cluster_input_map[cluster_idx]) {
        ...
        inputs.push_back(NodeBuilder::NodeOut(graph->FindNodeId(src_node_id),
                                              src_output_idx));
      }
    ...
    if (aot_requested) {
      ...
      if (!ngraph_tf_is_grappler_enabled()) {
        return errors::Internal(
            "AOT requested for non grappler build. Please use grappler build "
            "if "
            "AOT is required");
      }
      string input_node_type = "Placeholder";
      // In case of grappler, we have Placeholder, which might contain shape
      // info,
      // so it is possible we can aot without any provided shapes
      // in normal pass its args. unless shapes are provided there is no chance
      // of
      ...
      // This is a map of placeholder names and the shapes we can infer from
      // them
      ...
      for (auto node : graph->op_nodes()) {
        if (node->type_string() == input_node_type) {
          ...
          node_partial_shape_map.insert(
              {node->name(), partial_shape_from_node});
          ...
          if (node->type_string() == input_node_type) {
            ...
            if (shape_hint_for_node.is_valid()) {
              ...
              if (partial_shape_from_node.is_valid()) {
                ...
                NGRAPH_VLOG(5)
                    << "Partial shape from node is invalid. So using "
      ...
      for (auto node : graph->op_nodes()) {
        if (node->type_string() == "NGraphEncapsulate") {
          if (performed_aot_on_enc.find(node->name()) ==
            ...
            return errors::Internal(
                "Requested AOT, but did not perform AOT on ", node->name());
          }
        }
      }
